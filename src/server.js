...
const jsonStringToObject = require("./lib/jsonStringToObject");
...
server.unifiedServer = (request, response, protocol) => {
    // Get the URL and parse it.
    // const parsedUrl = url.parse(request.url, true);
    const parsedUrl = new url.URL(`${protocol}//${request.headers.host}${request.url}`);
    ...
    // TODO: Instead of changing it to an object we can change the 'actions' using this to use get method
        ...
        chosenController(requestData, controllerData)
            ...
            .then((responseContainer) => {
                ...
                if (responseContainer.statusCode === 200) {
                    debug(
                        "\x1b[32m%s\x1b[0m",
                        `${method.toUpperCase()} /${trimmedPath} ${responseContainer.statusCode}`
                    );
                } else {
                    debug(
                        "\x1b[31m%s\x1b[0m",
